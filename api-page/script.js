...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const populatePageContent = () => {
        ...
        const creator = settings.apiSettings?.creator || 'Franki';

        setPageContent(DOM.pageTitle, settings.name, "Kine API");
        setPageContent(DOM.wm, `© ${currentYear} ${creator}. Semua hak dilindungi.`);
        setPageContent(DOM.appName, settings.name, "Kine API");
        ...
        if (DOM.apiLinksContainer) {
            ...
            const defaultLinks = [{ url: "https://github.com/frankiofc/kine-api", name: "Lihat di GitHub", icon: "fab fa-github" }];
